...
BOOL CARPLayer::Send(unsigned char* ppayload, int nlength,int dev_num){
	...
	if(memcmp(routerDlg->m_IPLayer->GetDstIP(dev_num), broadcast, 4) == 0){
		//broadcast일 경우 바로 보냄
		return ((CEthernetLayer*)this->mp_UnderLayer)->Send(ppayload,nlength,ip_type,dev_num);
	}
	
	if((index = SearchIpAtTable(routerDlg->m_IPLayer->GetDstIP(dev_num))) != -1){
		//search 결과 존재하는경우.
		...
		//해당결과가 incomplete -- 보내지 않음
	}

	if(memcmp(routerDlg->m_IPLayer->GetDstIP(dev_num), routerDlg->m_IPLayer->GetSrcIP(dev_num), 4) == 0){
		//자신 ip로 보내는 경우
