...
BOOL CARPLayer::Send(unsigned char* ppayload, int nlength,int dev_num){
	...
	// MH: ARP로 내려온 패킷에서 IP Layer의 Destination IP Address가 Broadcast인 경우에는 ARP Table Search 할 필요 없이 바로 보낸다.
	...
	else if((index = SearchIpAtTable(routerDlg->m_IPLayer->GetDstIP(dev_num))) != -1){ //search 결과 존재하는경우.
		POSITION pos = Cache_Table.FindIndex(index);
		if(Cache_Table.GetAt(pos).cache_type == complete){ //해당 결과가 complete일 경우
			((CEthernetLayer *)GetUnderLayer())->SetDestinAddress(Cache_Table.GetAt(pos).Mac_addr, dev_num); //해당 mac을 설정
			return routerDlg->m_EthernetLayer->Send(ppayload,nlength,ip_type,dev_num); //그냥 보냄
		}
		else{ //해당결과가 incomplete -- 보내지 않음
			return false;
		}
	} else if(!memcmp(routerDlg->m_IPLayer->GetDstIP(dev_num), routerDlg->m_IPLayer->GetSrcIP(dev_num), 4)){ //자신 ip로 보내는 경우
